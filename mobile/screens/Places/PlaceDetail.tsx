...
const PlaceDetail = () => {
  const { params } = useRoute<RouteProp<PlaceDetailsProps, "PlaceDetails">>();
  ...
  const fetchPlace = async (placeId: string | undefined | null) => {
    if (placeId) {
      ...
      <ScrollView style={{ marginTop: 100 }}>
        ...
        <BodyContent>
          <Box flexDirection="row" justify="space-between">
            ...
            <TouchableOpacity
              onPress={() =>
                place?.contact ? Linking.openURL(`tel:${place?.contact}`) : null
              }
              style={{
          ...
          <LoadingContent condition={loadingPlace} placeholder={DescLoader}>
            <Box mt={20}>
